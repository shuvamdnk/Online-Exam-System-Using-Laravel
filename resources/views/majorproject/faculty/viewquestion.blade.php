...
<div>
  <p class="badge badge-pill badge-light shadow-sm"><a href="{{route('faculty')}}">Dashboard </a> > Questions</p>
</div>
  ...
  <h5 class="text-info">QUESTIONS</h5>
	<div class="row">
		...
		<div class="col-md-12">
    <a href="{{route('question.index')}}" style="margin-bottom: 15px;" class="btn btn-success shadow-sm">ADD QUESTION</a>

    <a href="#" style="margin-bottom: 15px;" class="btn btn-info shadow-sm">ALL QUESTION</a>
 
  <div class="scrollmenu">
<table id="example" class="table table-hover shadow-sm" style="background-color: #ffff;">
 ...
 @foreach($questions as $ques)
             <tr>
              <td>{{$ques->stream_name}}</td>
              <td>{{$ques->semester}}</td>
              <td>{{$ques->subject_name}}</td>
              <td>{{$ques->subject_code}}</td>
              <td>{{$ques->chapter_name}}</td>
              <td>{!! $ques->question !!}</td>
              <td>{!! $ques->option_a !!}</td>
              <td>{!! $ques->option_b !!}</td>
              <td>{!! $ques->option_c !!}</td>
              <td>{!! $ques->option_d !!}</td>
              <td>{{ $ques->answer }}</td>
              <td><a href="{{route('question.delete',['id'=>$ques->id])}}" class="btn btn-danger btn-block" onclick="return confirm('Are you sure you want to delete this item?');"><i class="fa fa-trash" aria-hidden="true"></i></a></td>
            </tr>
@endforeach
  </tbody>
</table>
