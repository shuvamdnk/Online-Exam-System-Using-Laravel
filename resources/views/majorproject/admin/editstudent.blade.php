...
<div class="container-fluid">
  ...
  <p class="badge badge-pill badge-light shadow-sm"><a href="{{route('admin')}}">Dashboard </a> > <a href="{{route('student.view')}}">Students</a> > Edit > {{$student->student_name}}</p>
	...
	<div class="row my-1 shadow-sm pt-3 pb-3 rounded" style="background-color: #ffff;">
		<div class="col-md-3 py-1">
			<div class="{{ $errors->has('student_name') ? ' has-error' : ''}}">
                    <input type="text" class="form-control shadow-sm rounded bg-light" id="name" aria-describedby="emailHelp" placeholder="Enter Student Name" name="student_name" onkeyup="myupper()" value="{{$student->student_name}}">
			</div>
		...
		<div class="col-md-3 py-1">
			<div class="{{ $errors->has('roll_number') ? ' has-error' : ''}}">
                  <input type="number" class="form-control shadow-sm rounded bg-light" placeholder="Roll number" name="roll_number" value="{{$student->roll_number}}">
        ...
        <div class="col-md-3 py-1">
        	<div class="{{ $errors->has('student_email') ? ' has-error' : ''}}">
        		<input type="email" class="form-control shadow-sm rounded bg-light" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Enter email" name="student_email" value="{{$student->student_email}}">
         ...
         <div class="col-md-3 py-1">
        	<div class="{{ $errors->has('phone_number') ? ' has-error' : ''}}">
        		<input type="number" class="form-control shadow-sm rounded bg-light" id="thresholdconfig" maxlength="10" aria-describedby="emailHelp" placeholder="Enter phone number" name="phone_number" value="{{$student->phone_number}}">
        	...
        	<div class="{{ $errors->has('student_stream') ? ' has-error' : ''}}">
        	 <select  class="form-control shadow-sm rounded bg-light"data-live-search="true" class="chosen" name="student_stream" title="Select Stream">
        	  @foreach($find as $found)
			       <option value="{{$found->id}}">{{$found->stream_name}}</option>
                 @endforeach
        ...
        <div class="col-md-3 py-1">
         ...
         <div class="col-md-3 py-1">
        	<div class="{{ $errors->has('session') ? ' has-error' : ''}}">
        	  <input type="text"  class="form-control shadow-sm rounded bg-light"  id="thresholdconfig" aria-describedby="emailHelp" placeholder="Enter Session" name="session" value="{{$student->session}}">
